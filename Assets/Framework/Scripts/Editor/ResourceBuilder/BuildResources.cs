...
namespace Framework.Editor
{
    ...
    public static class BuildResources
    {
        ...
        /// 运行生成资源。
        /// </summary>
        [MenuItem("Framework/Resource Tools/Build Resources", false, 40)]
        ...
        private static void Run(int? internalResourceVersion, Platform platforms, string outputDirectory, string buildEventHandlerTypeName)
        {
            ...
            if (!controller.Load())
            {
                Debug.LogError("Load configuration failure.");
            }
            else
            {
            ...
            {
                Debug.LogError(string.Format("Output directory '{0}' is invalid.", controller.OutputDirectory));
            }
